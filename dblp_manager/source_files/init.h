...
// item == <abc>xxx[<def>xxx</def>]xxx</abc>
// item_title == abc
// item_content == xxx[<def>xxx</def>]xxx
// tag == <abc>/<def>
// tag_content == abc/def
static inline void init(FILE *fp);

namespace namespace_init
{
    static inline uint64_t get_essay_num(FILE *fp);
    static inline void remove_item(FILE* fp, const char * const item_title);
    static inline void get_item_content(FILE* fp, const char * const item_title, char * buffer);
    static inline void get_month_from_content(const char * content, uint32_t &year_month);
    static inline void get_tag_content(FILE* fp,char *buffer);
    //更新作者列表，建立作者词索引
    static inline void update_author_list_author_index(const uint64_t essay_list_position, const std::vector<std::string>& author_name_temp_list);
    //static inline void* update_author_list_author_index_for_pthread(void *);
    //建立标题词索引
    static inline void update_essay_index(const uint64_t essay_list_position);
    //static inline void* update_essay_index_for_pthread(void *);
    static inline void get_top_100();
}

inline uint64_t namespace_init::get_essay_num(FILE *fp)
{
    uint64_t total=0;
    rewind(fp);
    ...
    while (1)
    {
        ...
        {
            my_error("xml解析出错：未知的文献类型");
        }
    }
    return total;
}

inline void init(FILE *fp)
{
    using namespace namespace_init;
        ...
        while (1)
        {
            ...
            else if(strcmp(temp_content, "title") == 0)
            {
                ...
                if(temp_content[0]!='\0')
                {
                    ...
                    {
                        my_error("xml解析出错：一个文献中有多个title");
                    }
            ...
            else if(strcmp(temp_content, "booktitle") == 0)
            {
                ...
                if(temp_content[0]!='\0')
                {
                    ...
                    {
                        my_error("xml解析出错：一个文献中有多个booktitle");
                    }
            ...
            else if(strcmp(temp_content, "journal") == 0)
            {
                ...
                if(temp_content[0]!='\0')
                {
                    ...
                    {
                        my_error("more than one journal in one essay!");
                    }
            ...
            else if(strcmp(temp_content, "volume")==0)
            {
                ...
                if (temp_content[0]!='\0')
                {
                    ...
                    {
                        my_error("more than one volume in an essay!");
                    }
...
inline void namespace_init::update_author_list_author_index(const uint64_t essay_list_position, const std::vector<std::string>& author_name_temp_list)
{
    //printf("1:%lu\n", essay_list_position);
...
inline void namespace_init::update_essay_index(const uint64_t essay_list_position)
{
...
inline void namespace_init::get_top_100()
{
...
inline void namespace_init::get_month_from_content(const char * content, uint32_t &year_month)
{
    char month[30];
    ...
    {
        my_error("xml解析出错：未知月份类型");
    }
}

inline void namespace_init::remove_item(FILE* fp, const char * const item_title)
{
...
inline void namespace_init::get_item_content(FILE* fp, const char * const item_title, char * buffer)
{
    const uint64_t title_len=strlen(item_title);
    while (1)
    {
...
//        |
// <xxxxx>
inline void namespace_init::get_tag_content(FILE* fp, char *buffer)
{
    fscanf(fp,"%*[^<]");
    getc(fp);
    while (1)
