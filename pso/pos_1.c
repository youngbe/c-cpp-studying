...
int main()
{
    ...
    init_my_random();
    const long double d = (W_MAX - W_MIN) / TIMES;
    ...
    for (size_t run_time = 0; run_time < RUNS; ++run_time) {
        ...
        for (size_t i = 0; i < POP_SIZE; ++i) {
            for (size_t i2 = 0; i2 < wei; ++i2) {
                ...
                // 初始化速度为范围的10%
                v[i][i2] = 0.1 * my_random(x_min - x_max, x_max - x_min);
            }
            pBest_v[i] = f(x[i], wei);
        ...
        for (size_t time = 0; time < TIMES; ++time) {
            ...
            for (size_t i = 0; i < POP_SIZE; ++i) {
                ...
                for (size_t i2 = 0; i2 < wei; ++i2) {
                    ...
                    // 设置速度上限为范围的10%
                    if (v[i][i2] > 0.1 * (x_max - x_min)) {
                        v[i][i2] = 0.1 * (x_max - x_min);
                    }
                    else if (v[i][i2] < 0.1 * (x_min - x_max)) {
                        v[i][i2] = 0.1 * (x_min - x_max);
                    }
                    // 当速度为0时给一个扰动
                    if (v[i][i2] == 0)
                        v[i][i2] = 0.01 * my_random(x_min - x_max, x_max - x_min);
                }
            }
            for (size_t i = 0; i < POP_SIZE; ++i) {
                // x[i] = x[i] + v[i]
